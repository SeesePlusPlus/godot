...
Kaldi::Kaldi() {
  ...
  this->wav_rspecifier = "scp:echo utterance-id1 MYFILE.wav";
...
void initialize() {
  ...
  nnet3::DecodableNnetSimpleLoopedInfo decodable_info(this->decodable_opts,
                                                      &this->am_nnet);

  fst::Fst<fst::StdArc> *decode_fst = ReadFstKaldiGeneric(this->fst_rxfilename);

  fst::SymbolTable *word_syms = NULL;
  if (this->word_syms_rxfilename != "")
    if (!(word_syms = fst::SymbolTable::ReadText(this->word_syms_rxfilename)))
      KALDI_ERR << "Could not read symbol table from file "
                << this->word_syms_rxfilename;

  this->spk2utt_reader = new SequentialTokenVectorReader(this->spk2utt_rspecifier);
  CompactLatticeWriter clat_writer(this->clat_wspecifier);
}
